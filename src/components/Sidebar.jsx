            ...
            <div className={`pt-12 pb-16 flex items-center transition-all duration-500 ${isCollapsed ? 'justify-center' : 'px-8'}`}>
                ...
                {!isCollapsed && (
                    ...
                    >
                        ...
                        <span className="text-[10px] font-black text-text-muted uppercase tracking-[0.4em] mt-1 block">Laboratory Core</span>
                    </motion.div>
                )}
            </div>
                ...
                ${isActive
                        ...
                        >
                            ...
                            <div className={`flex items-center transition-all duration-500 ${isCollapsed ? 'mx-auto' : 'px-6'}`}>
                                <item.icon size={22} className={isActive ? 'text-accent' : 'group-hover:text-accent transition-colors'} />
                                {!isCollapsed && (
                                    <span className="ml-5 font-black text-xs uppercase tracking-[0.25em] truncate">
                                        {item.label}
                                    </span>
                                )}
                            </div>
            ...
            <div className="p-4 border-t border-border-main">
                <div className="space-y-2">
                    ...
                    >
                        ...
                        {!isCollapsed && <span className="ml-5 font-black text-xs uppercase tracking-[0.25em]">{isDarkMode ? 'Light Mode' : 'Dark Mode'}</span>}
                    </button>
                    {!isCollapsed && (
                        <div className="p-4 bg-bg-surface rounded-2xl border border-border-main mb-4 flex items-center space-x-3">
                            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                            <span className="text-xs font-black text-text-muted uppercase tracking-widest truncate">{user?.email || 'DEBUG@AUTO'}</span>
                        </div>
                    )}
                    ...
                    >
                        ...
                        {!isCollapsed && <span className="ml-5 font-black text-xs uppercase tracking-[0.25em]">Logout</span>}
                    </button>
                </div>
            </div>
        </motion.aside>
